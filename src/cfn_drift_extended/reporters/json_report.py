...
from cfn_drift_extended.models import AuditReport, OrphanReport
...
class JsonReporter:
    ...
    def render_orphans(
        self, report: OrphanReport, output_path: Path | None = None
    ) -> str:
        """Serialize an orphan report to JSON.

        Args:
            report: The orphan report to serialize.
            output_path: Optional file path to write the JSON to.
        ...
        if output_path:
            output_path.parent.mkdir(parents=True, exist_ok=True)
            output_path.write_text(json_str, encoding="utf-8")
            logger.info("JSON orphan report written to %s", output_path)
